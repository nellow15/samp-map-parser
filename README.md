...
[![SA-MP](https://img.shields.io/badge/SA--MP-0.3.7-blue.svg?style=for-the-badge)](https://www.sa-mp.mp/)
...
A new AutoLoad system allows the server to automatically detect and load new map files from the `Maps` directory at a set interval — without requiring a server restart or manual command.

```pawn
public OnGameModeInit()
{
    ProcessMaps(); // load existing maps on startup

    // then start watching for new maps every 10 seconds
    StartAutoLoadMaps(10000);
}
```

You can also trigger a manual scan at any time using `AutoLoadMaps()`, which loads only files that are not already loaded. This is useful if you want fine-grained control over when the scan runs.

```pawn
CMD:refreshmaps(playerid, params[])
{
    if(!IsPlayerAdmin(playerid)) return SysMsg(playerid, "Unauthorized.");
    new loaded = AutoLoadMaps();
    SysMsg(playerid, "AutoLoad scan complete. %i new map(s) loaded.", loaded);
    return true;
}
```
...
| `StartAutoLoadMaps(interval = 5000)` | Starts a repeating timer that automatically scans the map directory and loads any new map files found. `interval` is the scan frequency in milliseconds (default: 5000). Returns `false` if AutoLoad is already running. |
| `StopAutoLoadMaps()` | Stops the AutoLoad timer. Returns `false` if AutoLoad was not running. |
| `IsAutoLoadRunning()` | Returns `true` if the AutoLoad timer is currently active, `false` otherwise. |
| `AutoLoadMaps()` | Manually scans the map directory and loads any files not already loaded. Returns the number of newly loaded maps. Can be called independently of the timer. |
...
| `PREVENT_REMOVE_BUILDING_CRASH`  | `1`  | When enabled (`1`), limits the number of `RemoveBuildingForPlayer` calls per player to 950 to avoid a known SA-MP crash. Set to `0` to disable this protection. |
